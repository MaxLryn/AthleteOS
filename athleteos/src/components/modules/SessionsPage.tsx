...
export default function SessionsPage({ sports, sessions, addSession, addSport, showToast }: Props) {
  ...
  return (
    <div>
      <Topbar title="Séances" subtitle={`${sessions.length} séance${sessions.length > 1 ? 's' : ''} enregistrée${sessions.length > 1 ? 's' : ''}`} action={{ label: 'Ajouter une séance', fn: () => setModal(true) }} />
      ...
      <div style={{ padding: '0 28px 28px' }}>
        ...
        {filtered.length === 0 ? (
          <Card style={{ textAlign: 'center', padding: '40px 20px' }}>
            ...
            <button onClick={() => setModal(true)} style={{ padding: '10px 20px', borderRadius: 9, background: 'var(--a1)', border: 'none', color: '#fff', fontSize: 13, cursor: 'pointer' }}>+ Ajouter une séance</button>
          </Card>
        ) : filtered.map(s => {
          const sp = sports.find(x => x.id === s.sport_id)
          return (
            <div key={s.id} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 20px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 16 }}>
      ...
      <SessionModal open={modal} onClose={() => setModal(false)} sports={sports} onSave={addSession} />
